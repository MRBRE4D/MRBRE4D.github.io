...
import style from './tw-zipcode.module.scss'
...
export default function TWZipCode({
  ...
  return (
    <>
      ...
      >
        <option value="-1" name="addr1">
          選擇縣市
        </option>
        {countries.map((value, index) => (
          <option key={index} value={index} name="addr1">
            {value}
          </option>
        ))}
      </select>
      <select
        ...
        name="addr2"
        className={style.selectTown}
      >
        <option value="-1" name="addr2">
          選擇區域
        </option>
        {countryIndex > -1 &&
          townships[countryIndex].map((value, index) => (
            <option key={index} value={index} name="addr2">
              {value}
            </option>
          ))}
      </select>
